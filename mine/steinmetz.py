...
from from_3b1b.on_hold.shadows import *
...
class Steinmetz(Sphere):
    CONFIG = {
        "checkerboard_colors": [GREEN_D, GREEN_E]
    }
    def func(self, u, v):
        if v >= 7*PI/4 or v < PI/4:
            return np.array([
                np.sin(u),
                np.sin(u)*np.tan(v),
                np.cos(u)
            ])
        elif v >= PI/4 and v < 3*PI/4:
            return np.array([
                np.sin(u)/np.tan(v),
                np.sin(u),
                np.cos(u)
            ])
        elif v >= 3*PI/4 and v < 5*PI/4:
            return np.array([
                -np.sin(u),
                -np.sin(u)*np.tan(v),
                np.cos(u)
...
class Nose(Sphere):
    ...
    def func(self, u, v):
        return np.array([
            np.cos(v)*(np.sqrt((2.1)**2-u**2)+np.exp(-u-2)*np.abs(2-u))/6,
            np.sin(v)*(np.sqrt((2.1)**2-u**2)+np.exp(-u-2)*np.abs(2-u))/6,
            u
        ])
...
class SphereCylinderScene(SpecialThreeDScene):
    ...
    def get_cylinder2(self, **kwargs):
        config = merge_dicts_recursively(self.sphere_config, kwargs)
        return Cylinder2(**config)
    
    def get_cylinder3(self, **kwargs):
        config = merge_dicts_recursively(self.sphere_config, kwargs)
        return Cylinder3(**config)

    def get_steinmetz(self, **kwargs):
        config = merge_dicts_recursively(self.sphere_config, kwargs)
        return Steinmetz(**config)
...
class SteinmetzScene(SphereCylinderScene):
    def construct(self):
        sphere = self.get_sphere()
        axes = self.get_axes()
        cylinder2 = self.get_cylinder2()
        cylinder3 = self.get_cylinder3()
        steinmetz = self.get_steinmetz()
        cylinder2.set_fill(opacity=0.75)
        cylinder3.set_fill(opacity=0.75)
        self.add(axes, cylinder2)
        self.wait(2)
        self.play(
            ShowCreation(cylinder3)
        )
        self.wait(3)
        ...
        self.add(steinmetz)
        self.wait(2)
        self.remove(cylinder2)
        self.play(
            ReplacementTransform(cylinder3, steinmetz)
        )
        self.wait(5)

class SteinmetzExplanation(SphereCylinderScene):
    def construct(self):
        sphere = Sphere(radius=2)
        axes = self.get_axes()
        cylinder2 = self.get_cylinder2()
        cylinder2.set_fill(opacity=0.5)
        cylinder3 = self.get_cylinder3()
        cylinder3.set_fill(opacity=0.5)
        self.add(axes, cylinder2, cylinder3)
        self.wait(2)
        self.play(
            ShowCreation(sphere)
        )
        self.wait(4)
        cut_sphere = Sphere(u_min=PI/4, radius=2)
        cut_sphere.set_fill(opacity=0.25)
        circle_1 = Circle(
            arc_center=2*np.cos(PI/4) * OUT, 
            radius=2*np.sin(PI/4),
            color=BLUE
        )
        cut_cylinder2 = Cylinder2(v_min=3*PI/4, v_max=TAU+PI/4, radius=2)
        cut_cylinder2.set_fill(opacity=0.25)
        line2_1 = Line(
            2 * (2*UP + np.cos(PI/4) * RIGHT + np.sin(PI/4) * OUT),
            2 * (2*DOWN + np.cos(PI/4) * RIGHT + np.sin(PI/4) * OUT),
            color=RED
        )
        line2_2 = Line(
            2 * (2*UP + np.cos(3*PI/4) * RIGHT + np.sin(3*PI/4) * OUT),
            2 * (2*DOWN + np.cos(3*PI/4) * RIGHT + np.sin(3*PI/4) * OUT),
            color=RED
        )
        cut_cylinder3 = Cylinder3(v_min=3*PI/4, v_max=TAU+PI/4, radius=2)
        cut_cylinder3.set_fill(opacity=0.25)
        line3_1 = Line(
            2 * (2*RIGHT + np.cos(PI/4) * UP + np.sin(PI/4) * OUT),
            2 * (2*LEFT + np.cos(PI/4) * UP + np.sin(PI/4) * OUT),
            color=MAROON
        )
        line3_2 = Line(
            2 * (2*RIGHT + np.cos(3*PI/4) * UP + np.sin(3*PI/4) * OUT),
            2 * (2*LEFT + np.cos(3*PI/4) * UP + np.sin(3*PI/4) * OUT),
            color=MAROON
        )
        self.play(
            ReplacementTransform(sphere, cut_sphere),
            ShowCreation(circle_1),
            ReplacementTransform(cylinder2, cut_cylinder2),
            ShowCreation(line2_1),
            ShowCreation(line2_2),
            ReplacementTransform(cylinder3, cut_cylinder3),
            ShowCreation(line3_1),
            ShowCreation(line3_2)
        )
        self.wait(11)
        self.begin_ambient_camera_rotation()
        self.move_camera(
        ...
        self.remove(cut_cylinder2, cut_cylinder3)
        steinmetz = self.get_steinmetz()
        steinmetz.set_fill(opacity=0.25)
        self.play(
            ShowCreation(steinmetz)
        )
        self.wait(2)
        for i in [2, 3]:
            self.remove(circle_1, line2_1, line2_2, line3_1, line3_2)
            circle_1 = Circle(
                arc_center=2*np.cos(i*PI/4) * OUT, 
                radius=2*np.sin(i*PI/4),
                color=BLUE
            )
            line2_1 = Line(
                2 * (2*UP + np.cos(PI/2-i*PI/4) * RIGHT + np.sin(PI/2-i*PI/4) * OUT),
                2 * (2*DOWN + np.cos(PI/2-i*PI/4) * RIGHT + np.sin(PI/2-i*PI/4) * OUT),
                color=RED
            )
            line2_2 = Line(
                2 * (2*UP + np.cos(PI/2+i*PI/4) * RIGHT + np.sin(PI/2+i*PI/4) * OUT),
            ...
            line3_1 = Line(
                2 * (2*RIGHT + np.cos(PI/2-i*PI/4) * UP + np.sin(PI/2-i*PI/4) * OUT),
                2 * (2*LEFT + np.cos(PI/2-i*PI/4) * UP + np.sin(PI/2-i*PI/4) * OUT),
                color=MAROON
            )
            line3_2 = Line(
                2 * (2*RIGHT + np.cos(PI/2+i*PI/4) * UP + np.sin(PI/2+i*PI/4) * OUT),
            ...
            self.add(circle_1, line2_1, line2_2, line3_1, line3_2)
            self.wait(2)

class Introduction(SpecialThreeDScene):
    def construct(self):
        self.set_camera_orientation(
            phi=70 * DEGREES,
            theta=-90 * DEGREES,
        ...
        self.play(
            FadeInFromDown(text1)
        )
        self.wait(4)
        self.add_fixed_in_frame_mobjects(text2)
        self.play(
            FadeInFromLarge(text2),
            ShowCreation(steinmetz),
            steinmetz.rotate, 
            20 * DEGREES
        )
...
class NoseScene(SpecialThreeDScene):
    def construct(self):
        self.set_camera_orientation(
            phi=70 * DEGREES,
            theta=-90 * DEGREES,
...
class CountOfThree(SpecialThreeDScene):
    def construct(self):
        self.set_camera_orientation(
            phi=70 * DEGREES,
            theta=-90 * DEGREES,
        ...
        tw.to_edge(UP)
        on.to_edge(UP)
        self.add_fixed_in_frame_mobjects(th)
        self.play(
            FadeInFromDown(th),
            ShowCreation(steinmetz),
            steinmetz.rotate, 
        ...
        self.wait(1)
        self.remove(th)
        self.add_fixed_in_frame_mobjects(tw)
        self.play(
            FadeInFromDown(tw),
            ShowCreation(steinmetz),
            steinmetz.rotate, 
        ...
        self.wait(1)
        self.remove(tw)
        self.add_fixed_in_frame_mobjects(on)
        self.play(
            FadeInFromDown(on),
            ShowCreation(steinmetz),
            steinmetz.rotate, 
...
class Ending(Scene):
    def construct(self):
        gjg = TextMobject("Gay Joy Go")
        gjg.set_color_by_gradient(RED, ORANGE, YELLOW, GREEN, BLUE, PURPLE)
        self.play(FadeInFromLarge(gjg))

class VolumeCalculation(SpecialThreeDScene):
    def construct(self):
        sa_equation = TexMobject(
            "\\text{(Square) : (Circle)} = 4:\\pi",
            tex_to_color_map={"R": BLUE}
        )
        sa_equation.to_edge(UP)
        ...
        sq.next_to(sa_equation, DOWN)
        circ.next_to(sa_equation,DOWN)

        self.add_fixed_in_frame_mobjects(sa_equation)
        self.play(
            Write(sq, stroke_width=1),
            Write(circ, stroke_width=1),
            FadeInFromDown(sa_equation)
        )
        self.play(
            sq.scale, 0.75,
            ...
            sq.shift, 2 * DOWN,
            circ.shift, 2 * DOWN,
            sa_equation.shift, 2 * DOWN
        )
        self.wait(2)
        vol_equation = TexMobject(
            "\\dfrac{4}{3}\\pi\\times\\dfrac{4}{\\pi}=\\dfrac{16}{3}"
        )
        vol_equation.scale(1.5)
        vol_equation.to_edge(UP)
        self.play(
            FadeInFromDown(vol_equation)
        )
